...
interface ModelDrawerProps {
  ...
  onCreateSchematicForModel: (model: SuzukiModel, presetCategory?: string) => void;
...
export const ModelDrawer: React.FC<ModelDrawerProps> = ({
  ...
  const linkedSchematics = useMemo(() => {
    return schematics.filter(s => s.applicableModelIds && s.applicableModelIds.includes(currentModelId));
          ...
          <div className="flex items-center gap-3.5">
            ...
            <div>
              <h2 className="text-xl font-black text-slate-900 font-display tracking-tight">
                {modelToEdit ? `Modelo: ${modelToEdit.name}` : 'Nuevo Modelo de Moto'}
              </h2>
              <p className="text-xs text-slate-500 font-mono">
                {modelToEdit ? `ID: ${modelToEdit.id} • ${linkedSchematics.length} despieces vinculados` : 'Especificaciones de compatibilidad OEM'}
              </p>
            </div>
          </div>
          <button
            ...
            <div className="space-y-6">
              ...
              <div className="p-5 rounded-2xl bg-slate-50 border border-slate-200 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-[11px] text-slate-500 mb-1 font-mono">Año Inicial</label>
                    <input
                      type="number"
                      min="1980"
                      max="2030"
                  ...
                  <div>
                    <label className="block text-[11px] text-slate-500 mb-1 font-mono">Año Final</label>
                    <input
                      type="number"
                      min="1980"
                      max="2030"
              ...
              <div className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 border border-slate-200">
                <div>
                  <p className="text-xs font-bold text-slate-900">Estado del Modelo</p>
                  <p className="text-[11px] text-slate-500 font-sans">Activa o desactiva la selección en el selector de garaje.</p>
                </div>
                <button
                  type="button"
              ...
              <div className="p-4 md:p-5 rounded-2xl bg-amber-50 border border-amber-200 flex flex-col sm:flex-row items-center justify-between gap-4">
                <div>
                  ...
                  <p className="text-xs text-amber-800 mt-0.5">
                    Crea un despiece desde cero en el lienzo interactivo con este modelo asignado.
                  </p>
                </div>
                <button
                  type="button"
                  onClick={() => {
                    if (modelToEdit) {
                      ...
                      setError('Primero debes guardar los datos del modelo antes de crearle despieces.');
                    }
                  }}
              ...
              {modelToEdit && unlinkedSchematics.length > 0 && (
                <div className="p-4 md:p-5 rounded-2xl bg-slate-50 border border-slate-200 space-y-3">
                  <div className="flex items-center gap-2 text-xs font-bold text-slate-900 font-mono uppercase">
                    ...
                    <span>Vincular Despiece Existente (Organizado por Categoría)</span>
                  </div>
                  <div className="flex flex-col sm:flex-row gap-2">
                    ...
                    >
                      <option value="">Selecciona un despiece para vincular a {name}...</option>
                      {groupedUnlinkedSchematics.map(([catGroup, items]) => (
                        <optgroup key={catGroup} label={`📁 ${catGroup.toUpperCase()}`}>
                          {items.map(s => (
              ...
              <div className="space-y-5">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-bold text-slate-900 font-mono uppercase">
                    Despieces Asignados a {name || 'este modelo'} ({linkedSchematics.length})
                  </span>
                ...
                {groupedLinkedSchematics.map(({ categoryName, schematicsList }) => (
                  <div
                    key={categoryName}
                    ...
                    <div className="flex flex-wrap items-center justify-between pb-2 border-b border-slate-100 gap-2">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] font-mono font-bold px-2 py-0.5 rounded bg-blue-50 text-[#0A3088] border border-blue-200">
                          {schematicsList.length} {schematicsList.length === 1 ? 'diagrama' : 'diagramas'}
                        </span>
                      </div>
                      ...
                      <button
                        ...
                        onClick={() => {
                          if (modelToEdit) {
                            onCreateSchematicForModel(modelToEdit, categoryName);
                          }
                        }}
                        ...
                        >
                          <div className="flex items-center gap-3 min-w-0">
                            <div className="w-12 h-10 rounded-lg bg-white border border-slate-200 flex items-center justify-center p-1 shrink-0">
                              {schematic.diagramImage ? (
                                <img src={schematic.diagramImage} alt={schematic.title} className="w-full h-full object-contain" />
                              ) : (
                                <Layers className="w-5 h-5 text-slate-400" />
                              )}
                            </div>
                            <div className="min-w-0">
                              <p className="font-extrabold text-slate-900 text-xs truncate font-display">{schematic.title}</p>
                          ...
                          <div className="flex items-center gap-1 shrink-0">
                            ...
                            <button
                              ...
                              onClick={() => {
                                if (onViewSchematic) {
                                  ...
                                  window.open(`/despieces?schematic=${schematic.id}`, '_blank');
                                }
                              }}
                            ...
                            <button
                              ...
                              onClick={() => {
                                if (currentModelId) {
                                  onUnlinkSchematic(schematic.id, currentModelId);
                                }
                              }}
                ...
                {linkedSchematics.length === 0 && (
                  <div className="text-center py-12 bg-slate-50 rounded-2xl border border-slate-200 p-6">
                    ...
                    <p className="font-bold text-slate-700 text-xs">No hay despieces asignados a este modelo</p>
                    <p className="text-[11px] text-slate-500 mt-0.5">
                      Usa el selector superior para vincular despieces existentes por categoría o crear uno nuevo.
                    </p>
                  </div>
                )}
              </div>
