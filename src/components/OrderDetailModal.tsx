...
export const OrderDetailModal: React.FC<OrderDetailModalProps> = ({
  ...
  const handleCancelOrder = async () => {
    ...
    if (!confirm('¿Estás seguro de anular este pedido pendiente? Se liberará la reserva de repuestos sin ningún costo.')) return;
    setIsCancellingOrder(true);
    try {
      const res = await saveReturnApi({
        orderId: order.id,
        ...
        reason: 'Anulación voluntaria de pedido pendiente',
        ...
        notes: 'Cancelación directa por el cliente desde el detalle de la orden.'
      });
      if (res.success) {
        order.status = 'Cancelado';
        if (order.id) {
          fetchReturns(undefined, undefined, order.id).then((returns) => {
  ...
  useEffect(() => {
    if (!isOpen || !order) {
      setRemainingPaymentSecs(0);
      return;
    }
    const isPend = order.status?.toLowerCase().includes("pendiente");
    const expiresAt = order.reservationExpiresAt || (isPend && order.date ? new Date(new Date(order.date).getTime() + 12 * 3600 * 1000).toISOString() : undefined);
    if (!expiresAt) {
      setRemainingPaymentSecs(0);
      return;
    }
    const updateCountdown = () => {
      ...
      setRemainingPaymentSecs(diff);
    };
    updateCountdown();
    const timer = setInterval(updateCountdown, 1000);
    return () => clearInterval(timer);
  }, [isOpen, order?.reservationExpiresAt, order?.status, order?.date]);
  ...
  const effectiveExpiresAt = order?.reservationExpiresAt || (isPending && order?.date ? new Date(new Date(order.date).getTime() + 12 * 3600 * 1000).toISOString() : undefined);
  const resHours = Math.floor(remainingPaymentSecs / 3600);
  const resMins = Math.floor((remainingPaymentSecs % 3600) / 60);
  const resSecs = remainingPaymentSecs % 60;
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              <div className="flex items-center gap-2.5 flex-wrap">
                ...
                {/* Header Remaining Countdown Chip */}
                {isPending && remainingPaymentSecs > 0 && (
                  <span className="text-[11px] font-mono font-black uppercase px-2.5 py-1 rounded-full bg-amber-500 text-slate-950 flex items-center gap-1.5 shadow-2xs animate-pulse">
                    <Clock className="w-3.5 h-3.5" />
                    <span>{String(resHours).padStart(2, '0')}:{String(resMins).padStart(2, '0')}:{String(resSecs).padStart(2, '0')} para pagar</span>
                  </span>
                )}
              </div>
          ...
          <div className="space-y-3">
            ...
            <div className="space-y-3">
              {order.items?.map((it: any, idx: number) => {
                ...
                const primaryOem = getPrimaryOem(partObj);
                const unitPrice = Number(partObj?.price ?? it?.unitPrice ?? it?.price ?? 0);
                const quantity = Number(it.quantity || 1);
                const itemTotal = unitPrice * quantity;
                const partName = partObj?.name || it?.name || "Repuesto Suzuki OEM";
                const partCategory = partObj?.category || it?.category || "Repuesto";
                const partImage = partObj?.image || partObj?.images?.[0] || it?.image || "";
                const bikeBrand =
                ...
                return (
                  ...
                  >
                    ...
                    <div className="flex items-start gap-3">
                      {shouldShowProductImages() && partImage ? (
                        <img
                          src={partImage}
                          alt={partName}
          ...
          <div className="bg-slate-50 p-5 rounded-2xl border border-slate-200 text-xs space-y-2 font-mono">
            ...
            <div className="pt-3">
              {activeReturn ? (
                ...
                <div className="p-4 bg-slate-100/90 border border-slate-200 rounded-2xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3 text-xs">
                  <div className="text-slate-600 font-sans">
                    <strong className="text-slate-900 font-display block text-xs">
                      ⏳ Pedido con Reserva Activa (Pendiente de Pago)
                    </strong>
                    <span className="text-[11px] text-slate-500">
                      Las garantías y devoluciones monetarias aplican únicamente para pedidos con pago verificado o entregados.
                    </span>
                  </div>
                  <button
                    type="button"
                    ...
                    className="px-4 py-2 bg-red-50 hover:bg-red-100 text-red-700 border border-red-200 rounded-xl text-xs font-black uppercase tracking-wider transition-colors cursor-pointer shrink-0 shadow-2xs"
                  >
                    {isCancellingOrder ? "Anulando..." : "🚫 Desistir / Anular"}
                  </button>
                </div>
              ) : (order?.status || "").toLowerCase().includes("cancelad") ||
                (order?.status || "").toLowerCase().includes("expirad") ||
                (order?.status || "").toLowerCase().includes("anulad") ? (
                <div className="p-3 bg-slate-100 border border-slate-200 rounded-xl text-xs text-slate-600 font-mono text-center">
                  🚫 Este pedido se encuentra cancelado o expirado (Sin cargos generados).
                </div>
              ) : (
                <button
                  type="button"
