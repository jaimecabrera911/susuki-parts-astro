...
export const CheckoutPage: React.FC<CheckoutPageProps> = ({
  ...
  const [selectedPaymentMethod, setSelectedPaymentMethod] = useState<"transferencia" | "wompi">("transferencia");
    ...
    fetchPaymentSettings()
      .then((data) => {
        if (data) {
          ...
          const activeAccounts = (data.bankTransfer?.accounts || []).filter((a: BankAccount) => a.active);
          const defaultAcc = activeAccounts.find((a: BankAccount) => a.isDefault) || activeAccounts[0];
          if (defaultAcc) {
            setSelectedBankAccountId(defaultAcc.id);
          }
          if (!data.bankTransfer?.enabled && data.wompi?.enabled) {
            setSelectedPaymentMethod("wompi");
          }
        }
      })
      .catch((err) => console.error("Error cargando medios de pago en checkout:", err));
  }, []);
      ...
      <div id="checkout-page" className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 sm:py-12">
        <div className="bg-white rounded-3xl border border-slate-200 shadow-xl overflow-hidden animate-in fade-in zoom-in-95 duration-300">
          ...
          <div className="p-6 sm:p-8 space-y-6">
            ...
            {/* Bank Details Card or Wompi Confirmation Banner */}
            {completedOrder.paymentMethod === "wompi" ? (
              <div className="bg-emerald-50 text-slate-900 rounded-2xl p-6 shadow-sm border border-emerald-200">
                <div className="flex items-center gap-3.5">
                  ...
                  <div>
                    <h3 className="text-base font-black font-display text-slate-900">
                      Pago Online Registrado (Wompi)
                    </h3>
                    <p className="text-xs text-slate-600 font-sans mt-0.5">
                      Transacción gestionada de forma segura a través de la pasarela Wompi Bancolombia.
                    </p>
                  </div>
                </div>
              </div>
            ) : (
              <div className="bg-slate-50 text-slate-900 rounded-2xl p-6 shadow-sm border border-slate-200 space-y-4">
                  ...
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-500 font-sans">
                        Transfiere el valor total usando tu número de pedido como referencia
                      </p>
                    </div>
                  </div>
          ...
          <form onSubmit={handleSubmitOrder} className="space-y-6">
            ...
            <div className="bg-white rounded-3xl p-6 sm:p-8 border border-slate-200 shadow-xs">
              ...
              <div className="space-y-4">
                {/* Wompi Online Payment Option (if enabled in DB) */}
                {paymentSettings?.wompi?.enabled && (
                  <div
                  ...
                  >
                    <input
                      ...
                      onChange={() => setSelectedPaymentMethod("wompi")}
                      className="mt-1 w-4 h-4 text-emerald-600 focus:ring-emerald-500 cursor-pointer"
                    />
                    <label htmlFor="pay-wompi" className="cursor-pointer w-full">
                      <div className="flex items-center justify-between gap-2">
                        <span className="font-black text-slate-900 text-sm font-display">
                          Pago Online Seguro (Wompi Bancolombia)
                        </span>
                        <span className="bg-emerald-100 text-emerald-800 text-[10px] font-black uppercase px-2 py-0.5 rounded font-mono">
                          Acreditación Inmediata
                        </span>
                      </div>
                      <p className="text-xs text-slate-600 mt-1">
                        Paga con Tarjetas de Crédito/Débito, PSE, Nequi o Botón Bancolombia mediante el widget oficial seguro de Wompi.
                      </p>
                    </label>
                  </div>
                )}

                {/* Transferencia Option Card (if enabled or default) */}
                {(!paymentSettings || paymentSettings?.bankTransfer?.enabled !== false) && (
                  <div
                  ...
                  >
                    <input
                      type="radio"
                      name="checkoutPaymentMethod"
                      id="pay-transferencia"
                      checked={selectedPaymentMethod === "transferencia"}
                      onChange={() => setSelectedPaymentMethod("transferencia")}
                      className="mt-1 w-4 h-4 text-[#E60012] focus:ring-[#E60012] cursor-pointer"
                    />
                    <label htmlFor="pay-transferencia" className="cursor-pointer w-full">
                      <div className="flex items-center justify-between gap-2">
                        <span className="font-black text-slate-900 text-sm font-display">
                      ...
                      <p className="text-[11px] text-slate-600 mt-2.5 leading-relaxed">
                        ...
                        <strong>"Pendiente de pago"</strong> y podrás enviar el soporte a nuestro WhatsApp para despacho inmediato.
                      </p>
                    </label>
                  </div>
                )}
