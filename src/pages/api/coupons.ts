  ...
  try {
    ...
    if (Array.isArray(body)) {
      for (const c of body) {
        ...
        const couponId = (c.id && !c.id.startsWith('coup-')) ? c.id : generateUuidV7('cpn');
        await db.insert(coupons).values({
          id: couponId,
    ...
    // Single upsert
    if (body.code) {
      const couponId = (body.id && !body.id.startsWith('coup-')) ? body.id : generateUuidV7('cpn');
      await db.insert(coupons).values({
        id: couponId,
